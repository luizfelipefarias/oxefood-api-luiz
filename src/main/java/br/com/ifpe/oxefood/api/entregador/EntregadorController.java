...
public class EntregadorController {
    ...
    // -ResponseEntity → significa que você não vai devolver só o objeto, mas uma
    // resposta HTTP completa podendo conter status http,corpo de resposta e
    // cabeçalhos extras como Location, content-type e etc.Caso tu use no retorno um
    // ResponseEntity<T>

    // -<Entregador> → indica o tipo do corpo da resposta (o conteúdo que vai dentro
    // do ResponseEntity).

    // -O @RequestBody faz o Spring pegar o JSON do corpo da requisição e converter
    // em objeto Java do tipo EntregadorRequest.
    public ResponseEntity<Entregador> save(@RequestBody EntregadorRequest entregadorRequest) {

        // O entregadorRequest é um objeto da classe EntregadorRequest, criado a partir
        // do JSON da requisição

        // Ele possui o método "build()", que cria e retorna um objeto da entidade
        // Entregador com os dados contidos no DTO

        // Salvamos essa entidade no banco através do entregadorService

        // O objeto salvo é armazenado em uma variável para ser retornado na resposta
        // HTTP com status 201 CREATED
